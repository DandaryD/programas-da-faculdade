...
// Função para salvar cidades em arquivo
void salvarCidades(const Cidade cidades[], int total) {
    FILE *f = fopen("supertrunfoteste2.txt", "w"); // Alterado aqui
    if (!f) {
        printf("Erro ao abrir arquivo para salvar!\n");
        return;
    }
    for (int i = 0; i < total; i++) {
        fprintf(f, "%s;%d;%s;%ld;%.2lf;%.2lf;%d;%.2lf;%.2lf\n",
            cidades[i].estado,
            cidades[i].codigo,
...
// Função para carregar cidades do arquivo
int carregarCidades(Cidade cidades[]) {
    FILE *f = fopen("supertrunfoteste2.txt", "r"); // Alterado aqui
    if (!f) return 0; // Se não existe, começa vazio
    int total = 0;
    while (fscanf(f, "%2[^;];%d;%99[^;];%ld;%lf;%lf;%d;%lf;%lf\n",
        cidades[total].estado,
        &cidades[total].codigo,
        cidades[total].nomeCidade,
        &cidades[total].populacao,
        &cidades[total].pib,
        &cidades[total].area,
        &cidades[total].numPontosTuristicos,
        &cidades[total].densidadePopulacional,
        &cidades[total].pibPerCapita
    ) == 9 && total < MAX_CIDADES) {
        total++;
    }
    fclose(f);
...
int main() {
    ...
    // Carrega cidades do "banco de dados"
    total = carregarCidades(cidades);
        ...
        switch (opcao) {
            case 1:
                if (total < MAX_CIDADES) {
                    ...
                    total++;
                    salvarCidades(cidades, total); // Salva após cadastrar
            ...
            case 3:
                ...
                salvarCidades(cidades, total); // Salva após remover
                break;
            case 0:
                printf("Saindo do programa...\n");
                salvarCidades(cidades, total); // Salva ao sair
                break;
            default:
                printf("Opcao invalida! Tente novamente.\n");
        }
